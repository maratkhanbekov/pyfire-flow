...
def upload_deck_media_files():
    path_from_current_folder_to_contents = '../../shared-decks/media/music-instruments/'
    shared_decks = SharedDeck(content_file_path=f'{path_from_current_folder_to_contents}content.xlsx')
    destination_path = 'shared-decks/music-instruments-v1/'
...
def upload_deck_content(destination_path, scale_mode=False):
    ...
    deck_instance['title'] = 'Learn Spanish: Music Instruments'
    deck_instance['colorHex'] = flashcards_firestore_schemes.colors['Reds'][0]
    deck_instance['cards'] = shared_deck.generate_cards_with_image_on_front_side_simple(col_with_text='back-text-esp')
