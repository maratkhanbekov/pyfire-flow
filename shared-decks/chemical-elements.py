...
def upload_deck_media_files():
    path_from_current_folder_to_contents = '../../shared-decks/media/chemical-elements/'
    shared_decks = SharedDeck(content_file_path=f'{path_from_current_folder_to_contents}content.xlsx')

    destination_path = 'shared-decks/chemical-elements-v1/'
    ...
    for i in range(len(shared_decks.content)):
        source_path_to_file = path_from_current_folder_to_contents + shared_decks.content['back-img'][i]
        filename = shared_decks.content['back-img'][i].split('/')[-1]
...
def upload_deck_content(destination_path, scale_mode=False):
    ...
    deck_instance['title'] = 'Learn Chemistry: Elements'
    deck_instance['colorHex'] = flashcards_firestore_schemes.colors['Purples'][0]
    deck_instance['cards'] = shared_deck.generate_cards_with_image_on_back_side_simple(col_with_text='front-text')
